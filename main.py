...
class Target:
    ...
    def update(self):
        if self.GROWTH_RATE + self.size >= self.MAX_SIZE:
            self.grow = False
...
def draw_top_bar(win, elapsed_time, targets_clicked, misses):
    pygame.draw.rect(win, "grey", (0, 0, WIDTH, TOP_BAR_HEIGHT))
    time_label = LABEL_FONT.render(f"Time: {format_time(elapsed_time)}", 1, "black")
    speed = round(targets_clicked / elapsed_time, 1)
    speed_label = LABEL_FONT.render(f"Speed: {speed} t/s", 1, "black")
    hits_label = LABEL_FONT.render(f"Hits: {targets_clicked} t/s", 1, "black")
...
def end_screen(win, elapsed_time, targets_clicked, clicks):
    ...
    hits_label = LABEL_FONT.render(f"Hits: {targets_clicked} t/s", 1, "white")
...
def main():
    ...
    while run:
        ...
        draw(WIN, targets)
        draw_top_bar(WIN, elapsed_time, targets_clicked, misses)
        pygame.display.update()
    pygame.quit()
